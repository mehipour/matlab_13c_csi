function img = csi_dynamic_reconphased_20181022_carbon(complex_mrsi,data_path,lb,debluring ,fastrun,flow,lac_bigger_than_pyr,image_num,RE,IM)


%% Syntax: 
% img = csireconphased_20160912_steve(data_path,lb,debluring,fastrun,flow,lac_bigger_than_pyr,image_num)
%
...
%    end
   ...
   % identity strongest line in k=0 spectrum as pyruvate
    k0norm = normn(squeeze(sum(sum(fti,1),2)));
...
%        ph1 = 13;  
       ...
       p = polyfit(xscalenopeaks,spectprimephasednopeaks,4);
       warning(ws);
...
%        plot(spectprimephased);
   ...
   if ~debluring
        ...
        save_name  = [data_path(1:temp(end-1)) 'deblur0' data_path(temp(end):end) num2str(image_num)];
   else
       if ~exist([data_path(1:temp(end-1)) 'deblur1/'],'dir')
            mkdir(data_path(1:temp(end-1)),'deblur1')
       end
       save_name  = [data_path(1:temp(end-1)) 'deblur1' data_path(temp(end):end) num2str(image_num)];
